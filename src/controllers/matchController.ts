...
export const submitPlayerRatings = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ error: 'Unauthorized' });
      return;
    ...
    for (const rating of ratings) {
      if (!rating.playerId || !rating.rating || rating.rating < 1 || rating.rating > 5) {
        res.status(400).json({ error: 'Invalid rating data. Each rating must have playerId and rating (1-5)' });
        return;
      }
    }

    // Add rater ID to each rating
    const ratingsWithRater = ratings.map(rating => ({
      ...rating,
      raterId: req.user!.id
    }));

    await database.submitPlayerRatings(matchId, ratingsWithRater);

    console.log('✅ Player ratings submitted successfully');
    res.json({ 
      message: 'Player ratings submitted successfully',
...
export const getMatchRatings = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({ error: 'Unauthorized' });
      return;
    ...
    console.log('📊 Getting ratings for match:', matchId);
    const ratings = await database.getMatchRatings(matchId);
